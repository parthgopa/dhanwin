...
import { X, Lock, Phone, Mail, User as UserIcon, ArrowRight, ShieldCheck, KeyRound, Gift, Eye, EyeOff, CheckCircle2, RotateCw, Sparkles } from 'lucide-react';
...
export const AuthModal = ({ isOpen, onClose, initialMode = 'login' }) => {
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      showToast('Welcome to Dhanwin! Your account is ready.', 'success');
      resetAndClose();
    } catch (err) {
      showToast(err.message, 'error');
    } finally {
        ...
        <div className="flex items-center justify-between border-b border-[#232b3b] pb-3.5 mb-4">
          <div>
            ...
            <p className="text-xs text-gray-400">
              {isRegister
                ? (regStep === 2 ? 'Enter 6-digit code sent to your email' : 'Create an Account & Start Playing')
        ...
        {isRegister && regStep === 1 && (
          <form onSubmit={handleRequestOTP} className="space-y-3">
            <div className="p-2.5 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-center gap-2 text-xs text-amber-300">
              ...
              <span>Instant 24/7 Deposits & Fast Payouts!</span>
            </div>
